      ...
      if(iLayer.eq.1) then
          ...
          if((GP%CoordinatesType.eq.0).and.(x(1).ge.0).and.(x(1).le.180.0)) then
              GP%StartEastWest = 1
          else if((GP%CoordinatesType.eq.0).and.(x(1).gt.180.0.or.x(1).lt.0.0)) then
              GP%StartEastWest = 2
          endif
      endif
      ...
      call handleCDFError(NF90_OPEN(TRIM(ADJUSTL(GP%InitialConditionFileName)), NF90_NOWRITE, fid))

      vid = 3; ! vid = 3: z values, float (real*4)
      ALLOCATE(z(LP(1)%NX, LP(1)%NY))
      call handleCDFError(NF90_GET_VAR(fid, vid, z))
      do i = 1, LP(1)%NX
          do j = 1, LP(1)%NY
              if(LP(iLayer)%Z(i,j).gt.GP%WaterDepthLimit) then
                  LP(iLayer)%H(1,i,j) = DBLE(z(i,j))
              endif
          enddo
      enddo
