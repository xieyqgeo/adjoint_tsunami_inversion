...
type  ::  StationParameters
    ...
    !**** For Time Reversal ****!
    character*999  ::  TR_DataPath           ! DataPath
    integer*4  ::  IX, IY                    ! located within IX~IX+1 and IY~IY+1
    integer*4  ::  TR_NR = 10                ! Raidus = TR_NR*dt*SQRT(gZ)
    integer*4  ::  TR_PointSourceType = 2    ! 1:circular 2:Gaussian
    integer*4  ::  TR_AmpCorrection = 1      ! 0:no correction; 1:correction based on SQRT(r); 2:normalized by maximum
    real*8     ::  TR_EpiX, TR_EpiY          ! Correction reference point
    real*8     ::  TR_Weight = 1.0           ! If = 0.0 then not used
    integer*4  ::  TR_NPoints                ! How many grids within this point source
    integer*4, allocatable  ::  TR_Points(:,:)      ! Coordinates of those points
    real*8, allocatable     ::  TR_PointsValue(:,:) ! Values in time
    real*8                  ::  TR_HMax             ! Max Absolute Value
