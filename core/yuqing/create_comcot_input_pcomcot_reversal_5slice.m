function create_comcot_input_pcomcot_reversal_5slice(init_cond,total_time,time_step,time_slice)
% create pcomcot.ctl for time reversal
if length(time_slice)>1
    dtime_save=time_slice(2)-time_slice(1);
else
    dtime_save=total_time;
end
start_save=total_time-time_slice(end);
...
fprintf(fid,'%s %f\n', ' Time start to record snapshot          (second):   ',start_save);
fprintf(fid,'%s %f\n',' Time interval to Save Data             (second):  ',dtime_save);
