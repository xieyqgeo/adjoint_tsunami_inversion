function create_comcot_input_pcomcot_forward_5slice(init_cond,time_step,save_time)
...
fprintf(fid,'%s %f\n',' Total run time                         (second):  ',save_time);
fprintf(fid,'%s %f\n',' Time step                              (second):   ',time_step);
fprintf(fid,'%s \n', 'Time start to record snapshot          (second):   0');
fprintf(fid,'%s %f\n',' Time interval to Save Data             (second):  ',save_time);
